    ...
    const int iRotation, const bool bFlip)
{
    ...
    hr = FillVertexBuffer(m_pDeviceContext, m_pVertexBuffer, Tex.desc.Width, Tex.desc.Height, srcRect, iRotation,
                          bFlip);
    if (FAILED(hr))
    {
        return hr;
    ...
    TextureBlt11(m_pDeviceContext, pRenderTargetView, VP, m_pVSimpleInputLayout, m_pVS_Simple, pPixelShader,
                 Tex.pShaderResource, m_pSamplerPoint, pConstantBuffer, m_pVertexBuffer);
    ...
    const int iRotation, const bool bFlip)
{
    ...
    TextureBlt11(m_pDeviceContext, pRenderTargetView, VP, m_pVSimpleInputLayout, m_pVS_Simple, pPixelShader,
                 Tex.pShaderResource, m_pSamplerPoint, m_pResizeShaderConstantBuffer, m_pVertexBuffer);
...
static bool ToggleHDR(const DisplayConfig_t& displayConfig, const bool bEnableAdvancedColor)
{
    ...
    if (IsWindows11_24H2OrGreater())
    {
        ...
        DLogIf(ERROR_SUCCESS != ret,
               L"ToggleHDR() : DisplayConfigSetDeviceInfo(DISPLAYCONFIG_SET_HDR_STATE) with '{}' failed with error {}",
               bEnableAdvancedColor, HR2Str(HRESULT_FROM_WIN32(ret)));
    }
    else
    {
        ...
        DLogIf(ERROR_SUCCESS != ret,
               L"ToggleHDR() : DisplayConfigSetDeviceInfo(DISPLAYCONFIG_SET_ADVANCED_COLOR_STATE) with '{}' failed with error {}",
               bEnableAdvancedColor, HR2Str(HRESULT_FROM_WIN32(ret)));
    }
    ...
    if (ret == ERROR_SUCCESS && beforeModeOpt.has_value())
    {
        ...
        if (afterModeOpt.has_value())
        {
            ...
            if (beforeMode.dmPelsWidth != afterMode.dmPelsWidth || beforeMode.dmPelsHeight != afterMode.dmPelsHeight
                || beforeMode.dmBitsPerPel != afterMode.dmBitsPerPel || beforeMode.dmDisplayFrequency != afterMode.
                dmDisplayFrequency)
            {
                ...
                auto ret = ChangeDisplaySettingsExW(displayConfig.displayName, &beforeMode, nullptr, CDS_FULLSCREEN,
                                                    nullptr);
                DLogIf(DISP_CHANGE_SUCCESSFUL != ret, L"ToggleHDR() : ChangeDisplaySettingsExW() failed with error {}",
                       HR2Str(HRESULT_FROM_WIN32(ret)));
            }
        }
    }
    ...
    for (const auto& [displayName, state] : m_hdrModeSavedState)
    {
        ...
        if (GetDisplayConfig(displayName.c_str(), displayConfig))
        {
            if (displayConfig.HDRSupported() && displayConfig.HDREnabled() != state)
            {
                ...
                DLogIf(!ret, L"CDX11VideoProcessor::~CDX11VideoProcessor() : Toggle HDR {} for '{}' failed",
                       state ? L"ON" : L"OFF", displayName);
            }
        }
    }
...
HRESULT CDX11VideoProcessor::Init(const HWND hwnd, const bool displayHdrChanged, bool* pChangeDevice/* = nullptr*/)
{
    ...
    DLog(L"CDX11VideoProcessor::Init() : D3D11CreateDevice() successfully with feature level {}.{}",
         (featurelevel >> 12), (featurelevel >> 8) & 0xF);
...
HRESULT CDX11VideoProcessor::SetShaderDoviCurvesPoly()
{
    ...
    {
        D3D11_BUFFER_DESC BufferDesc = {
            sizeof(polyCurves), D3D11_USAGE_DYNAMIC, D3D11_BIND_CONSTANT_BUFFER, D3D11_CPU_ACCESS_WRITE, 0, 0
        };
...
HRESULT CDX11VideoProcessor::SetShaderDoviCurves()
{
    ...
    {
        D3D11_BUFFER_DESC BufferDesc = {
            sizeof(cbuffer), D3D11_USAGE_DYNAMIC, D3D11_BIND_CONSTANT_BUFFER, D3D11_CPU_ACCESS_WRITE, 0, 0
        };
        D3D11_SUBRESOURCE_DATA InitData = {&cbuffer, 0, 0};
        hr = m_pDevice->CreateBuffer(&BufferDesc, &InitData, &m_pDoviCurvesConstantBuffer);
    }
...
void CDX11VideoProcessor::UpdateTexParams(int cdepth)
{
    ...
    {
    case TEXFMT_AUTOINT:
        m_InternalTexFmt = (cdepth > 8 || m_bVPUseRTXVideoHDR)
                               ? DXGI_FORMAT_R10G10B10A2_UNORM
                               : DXGI_FORMAT_B8G8R8A8_UNORM;
        break;
    case TEXFMT_8INT: m_InternalTexFmt = DXGI_FORMAT_B8G8R8A8_UNORM;
        break;
    case TEXFMT_10INT: m_InternalTexFmt = DXGI_FORMAT_R10G10B10A2_UNORM;
        break;
    case TEXFMT_16FLOAT: m_InternalTexFmt = DXGI_FORMAT_R16G16B16A16_FLOAT;
        break;
    default:
        ASSERT(FALSE);
    }
...
void CDX11VideoProcessor::CalcStatsParams()
{
    if (m_pDeviceContext && !m_windowRect.IsRectEmpty())
    {
        ...
        for (int y = m_GraphRect.top + (m_Yaxis - m_GraphRect.top) % (linestep); y < m_GraphRect.bottom; y += linestep)
        {
            ...
            m_Lines.AddPoints(points, std::size(points),
                              (y == m_Yaxis) ? D3DCOLOR_XRGB(150, 150, 255) : D3DCOLOR_XRGB(100, 100, 255));
        }
    ...
    if (m_TexSrcVideo.pTexture2)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                const int cromaPitch = (m_TexSrcVideo.pTexture3)
                                           ? srcPitch / m_srcParams.pDX11Planes->div_chroma_w
                                           : srcPitch;
                ...
                if (m_TexSrcVideo.pTexture3)
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        m_pCopyPlaneFn(cromaH, (BYTE*)mappedResource.pData, mappedResource.RowPitch, srcData,
                                       cromaPitch);
...
HRESULT CDX11VideoProcessor::SetDevice(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        m_strAdapterDescription = std::format(L"{} ({:04X}:{:04X})", dxgiAdapterDesc.Description,
                                              dxgiAdapterDesc.VendorId, dxgiAdapterDesc.DeviceId);
    ...
    D3D11_BUFFER_DESC BufferDesc = {
        sizeof(VERTEX) * 4, D3D11_USAGE_DYNAMIC, D3D11_BIND_VERTEX_BUFFER, D3D11_CPU_ACCESS_WRITE, 0, 0
    };
    ...
    HRESULT hr4 = m_TexDither.Create(m_pDevice, DXGI_FORMAT_R16G16B16A16_FLOAT, dither_size, dither_size,
...
HRESULT CDX11VideoProcessor::InitSwapChain(bool bWindowChanged)
{
    ...
    if (bFullscreenChange || bWindowChanged)
    {
        ...
        if ((m_bHdrPassthrough || m_bHdrLocalToneMapping) && SourceIsPQorHLG())
        {
            ...
            {
                DLog(
                    L"CDX11VideoProcessor::InitSwapChain() - SwapChain was created during the call to InitMediaType(), exit");
                return S_OK;
            }
        }
    }

    const auto bHdrOutput = m_bHdrSupport && (m_bHdrPassthrough || m_bHdrLocalToneMapping) && (SourceIsHDR() ||
        m_bVPUseRTXVideoHDR);
    ...
    if (m_bIsFullscreen)
    {
        ...
        if ((m_iSwapEffect == SWAPEFFECT_Flip && IsWindows8OrGreater()) || bHdrOutput)
        {
            ...
            desc1.SwapEffect = IsWindows10OrGreater()
        ...
        hr = m_pDXGIFactory2->CreateSwapChainForHwnd(m_pDevice, m_hWnd, &desc1, &fullscreenDesc, nullptr,
                                                     &m_pDXGISwapChain1);
        g_bCreateSwapChain = false;
        DLogIf(FAILED(hr),
               L"CDX11VideoProcessor::InitSwapChain() : CreateSwapChainForHwnd(fullscreen) failed with error {}",
               HR2Str(hr));
    ...
    else
    {
        ...
        DLogIf(FAILED(hr), L"CDX11VideoProcessor::InitSwapChain() : CreateSwapChainForHwnd() failed with error {}",
               HR2Str(hr));
...
bool CDX11VideoProcessor::HandleHDRToggle()
{
    ...
    {
        MONITORINFOEXW mi = {sizeof(mi)};
        GetMonitorInfoW(m_lastFullscreenHMonitor
                            ? m_lastFullscreenHMonitor
                            : MonitorFromWindow(m_hWnd, MONITOR_DEFAULTTOPRIMARY), (MONITORINFO*)&mi);
        DisplayConfig_t displayConfig = {};
        ...
        if (GetDisplayConfig(mi.szDevice, displayConfig))
        {
            if (displayConfig.HDRSupported() && m_iHdrToggleDisplay)
            {
                ...
                (m_iHdrToggleDisplay == HDRTD_On || m_iHdrToggleDisplay == HDRTD_OnOff
                    || m_bIsFullscreen && (m_iHdrToggleDisplay == HDRTD_On_Fullscreen || m_iHdrToggleDisplay ==
    ...
    else if (m_iHdrToggleDisplay)
    {
        ...
        if (GetDisplayConfig(mi.szDevice, displayConfig))
        {
            ...
            if (displayConfig.HDRSupported() && displayConfig.HDREnabled() &&
                (!bWindowsHDREnabled || (m_iHdrToggleDisplay == HDRTD_OnOff || m_iHdrToggleDisplay ==
                    HDRTD_OnOff_Fullscreen && m_bIsFullscreen)))
            {
                bRet = ToggleHDR(displayConfig, false);
                DLogIf(!bRet, L"CDX11VideoProcessor::HandleHDRToggle() : Toggle HDR OFF failed");
...
BOOL CDX11VideoProcessor::InitMediaType(const CMediaType* pmt)
{
    ...
    if (!VerifyMediaType(pmt)) { return FALSE; }

    ReleaseVP();
    m_activeHdrMode = HdrMode::UNKNOWN;
    AutoSwapLog(L"InitMediaType: reset activeHdr; settings pass=%d localTM=%d type=%d",
    ...
    if (pmt->formattype == FORMAT_VideoInfo2)
    {
        ...
        if (FmtParams.CSType == CS_YUV && (vih2->dwControlFlags & (AMCONTROL_USED | AMCONTROL_COLORINFO_PRESENT)))
        {
            ...
            m_decExFmt.SampleFormat = AMCONTROL_USED | AMCONTROL_COLORINFO_PRESENT;
        }
    ...
    case CF_NV12:
    case CF_RGB24:
    case CF_BGR48: m_srcPitch = ALIGN(m_srcPitch, 4);
        break;
    }
    if (pBIH->biCompression == BI_RGB && pBIH->biHeight > 0) { m_srcPitch = -m_srcPitch; }
    ...
    if (pmt->FormatLength() == 112 + sizeof(VR_Extradata))
    {
        ...
        if (vrextra->QueryWidth == pBIH->biWidth && vrextra->QueryHeight == pBIH->biHeight && vrextra->Compression ==
            pBIH->biCompression)
        {
    ...
    if (m_srcRect.IsRectNull()) { m_srcRect.SetRect(0, 0, origW, origH); }
    ...
    switch (FmtParams.cformat)
    {
    case CF_NV12: disableD3D11VP = !m_VPFormats.bNV12;
        break;
    case CF_P010:
    case CF_P016: disableD3D11VP = !m_VPFormats.bP01x;
        break;
    case CF_YUY2: disableD3D11VP = !m_VPFormats.bYUY2;
        break;
    default: disableD3D11VP = !m_VPFormats.bOther;
        break;
    }
    if (m_srcExFmt.VideoTransferMatrix == VIDEOTRANSFERMATRIX_YCgCo || m_Dovi.bValid) { disableD3D11VP = true; }
    if (FmtParams.CSType == CS_RGB && m_VendorId == PCIV_NVIDIA) { disableD3D11VP = true; }
    if (disableD3D11VP) { FmtParams.VP11Format = DXGI_FORMAT_UNKNOWN; }
    ...
    if (m_bHdrAllowSwitchDisplay && m_srcVideoTransferFunction != m_srcExFmt.VideoTransferFunction)
    {
        ...
        if (!ret && ((m_bHdrPassthrough || m_bHdrLocalToneMapping) && m_bHdrSupport && SourceIsPQorHLG() && !
            m_pDXGISwapChain4)) { ret = true; }
        if (ret)
        {
            ReleaseSwapChain();
            Init(m_hWnd, false);
    ...
    if (FmtParams.VP11Format != DXGI_FORMAT_UNKNOWN)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (m_srcExFmt.VideoTransferFunction == MFVideoTransFunc_2084 && !(m_bHdrSupport && (m_bHdrPassthrough ||
                m_bHdrLocalToneMapping)) && m_bConvertToSdr)
            {
            ...
            if (m_bHdrSupport && m_bHdrLocalToneMapping)
            {
                ...
                DLogIf(m_pPSHDR10ToneMapping,
                       L"CDX11VideoProcessor::InitMediaType() m_pPSHDR10ToneMapping(type: '{}') created",
    ...
    if (SUCCEEDED(hr))
    {
        ...
        AutoSwapLog(L"InitMediaType: pipeline set to %s (bHdrOutput=%d; TF=%d)",
                    (m_activeHdrMode == HdrMode::HDR ? L"HDR" : L"SDR"), (int)bHdrOutput,
                    (int)m_srcExFmt.VideoTransferFunction);
        DLog(L"CDX11VideoProcessor::InitMediaType() - Pipeline configured for: %s",
             (m_activeHdrMode == HdrMode::HDR ? L"HDR" : L"SDR"));

        // Present one black frame after (re)config to avoid showing uninitialized content
        if (m_pFilter) { m_pFilter->Redraw(); }
        // If you prefer inline: replace the line above with:
        // if (m_pDXGISwapChain1) { FillBlack(); }
                                               ...
                                               const CMediaType* pmt)
{
    ...
    DLog(L"CDX11VideoProcessor::InitializeD3D11VP() started with input surface: {}, {} x {}",
         DXGIFormatToString(dxgiFormat), width, height);

    m_TexSrcVideo.Release();

    const bool bHdrEnabled = m_bHdrDisplayModeEnabled && (SourceIsPQorHLG() || (m_bVPUseRTXVideoHDR && params.CDepth ==
        8));
    m_D3D11OutputFmt = m_InternalTexFmt;
    HRESULT hr = m_D3D11VP.InitVideoProcessor(dxgiFormat, width, height, m_srcExFmt, m_bInterlaced, bHdrEnabled,
                                              m_D3D11OutputFmt);
    if (FAILED(hr))
    {
    ...
    auto rtxHDR = m_bVPRTXVideoHDR && m_bHdrSupport && (m_bHdrPassthrough || m_bHdrLocalToneMapping) && m_iTexFormat !=
        TEXFMT_8INT && !SourceIsHDR();
...
HRESULT CDX11VideoProcessor::InitializeTexVP(const FmtConvParams_t& params, const UINT width, const UINT height)
{
    ...
    DLog(L"CDX11VideoProcessor::InitializeTexVP() started with input surface: {}, {} x {}",
         DXGIFormatToString(srcDXGIFormat), width, height);

    HRESULT hr = m_TexSrcVideo.CreateEx(m_pDevice, srcDXGIFormat, params.pDX11Planes, width, height,
                                        Tex2D_DynamicShaderWrite);
    if (FAILED(hr))
    {
    ...
    EXECUTE_ASSERT(
        S_OK == CreateVertexBuffer(m_pDevice, &m_PSConvColorData.pVertexBuffer, m_srcWidth, m_srcHeight, m_srcRect, 0,
            false));
    ...
    if (VerifyMediaType(&mt))
    {
        ...
        else
        {
            ...
            if (!m_Alignment.texture.pTexture)
            {
                ...
                if (VP11Format != DXGI_FORMAT_UNKNOWN)
                {
                    hr = m_Alignment.texture.Create(m_pDevice, VP11Format, biWidth, biHeight,
                                                    Tex2D_DynamicShaderWriteNoSRV);
                }
                if (FAILED(hr) && FmtParams.DX11Format != DXGI_FORMAT_UNKNOWN)
                {
                    hr = m_Alignment.texture.CreateEx(m_pDevice, FmtParams.DX11Format, FmtParams.pDX11Planes, biWidth,
                                                      biHeight, Tex2D_DynamicShaderWrite);
                }
                if (FAILED(hr))
                {
                    return FALSE;
                ...
                UINT RowPitch = 0;
                D3D11_MAPPED_SUBRESOURCE mappedResource = {};
                if (SUCCEEDED(m_pDeviceContext->Map(m_Alignment.texture.pTexture, 0, D3D11_MAP_WRITE_DISCARD, 0,
                                                    &mappedResource)))
                {
...
HRESULT CDX11VideoProcessor::CopySample(IMediaSample* pSample)
{
    ...
    bool updateStats = false; // FIXED: Declare updateStats variable
    m_hdr10 = {};
    ...
    if (CComQIPtr<IMediaSideData> pMediaSideData = pSample)
    {
        ...
        {
            const MediaSideDataDOVIMetadata* pDOVIMetadata = nullptr; // FIXED: Changed from pDOVI to pDOVIMetadata
            size = 0;
            hrLocal = pMediaSideData->GetSideData(IID_MediaSideDataDOVIMetadata,
                                                  reinterpret_cast<const BYTE**>(&pDOVIMetadata), &size);
            if (SUCCEEDED(hrLocal) && size == sizeof(MediaSideDataDOVIMetadata) && CheckDoviMetadata(pDOVIMetadata, 1))
            {
                ...
                (memcmp(
                    ...
                    sizeof(pDOVIMetadata->ColorMetadata.ycc_to_rgb_matrix)) != 0) || // FIXED: Added sizeof()
                (memcmp(
                ...
                if (bMasteringLuminanceChanged)
                {
                    ...
                    m_DoviMaxMasteringLuminance = static_cast<UINT>(pl_hdr_rescale(
                        m_Dovi.msd.ColorMetadata.source_max_pq / 4095.f));
                    m_DoviMinMasteringLuminance = static_cast<UINT>(pl_hdr_rescale(
                        m_Dovi.msd.ColorMetadata.source_min_pq / 4095.f) * 10000.0);
                }
                ...
                if (bMappingCurvesChanged)
                {
                    HRESULT hrCurves; // FIXED: Declare local hr variable
                    if (m_Dovi.bHasMMR)
                    {
                        hrCurves = SetShaderDoviCurves();
                    }
                    else
                    {
                        hrCurves = SetShaderDoviCurvesPoly();
                    }
                }
    ...
    {
        // HDR content: use Enhanced ACES (type 5) local tone mapping
        idealPassthrough = false;
        idealLocalTM = true;
        idealType = 5; // Enhanced ACES (was ACEScg)
    ...
    {
        AutoSwapLog(
            L"CopySample: Auto-swap trigger. Content=%s, current: pass=%d localTM=%d type=%d maxNits=%.1f -> ideal: pass=%d localTM=%d type=%d maxNits=%.1f",
            detectedHDR ? L"HDR" : L"SDR",
            (int)curSets.bHdrPassthrough, (int)curSets.bHdrLocalToneMapping, (int)curSets.iHdrLocalToneMappingType,
            curSets.fHdrDisplayMaxNits,
        ...
        AutoSwapLog(L"CopySample: Settings applied. New: pass=%d localTM=%d type=%d maxNits=%.1f",
                    (int)curSets.bHdrPassthrough, (int)curSets.bHdrLocalToneMapping,
                    (int)curSets.iHdrLocalToneMappingType, curSets.fHdrDisplayMaxNits);
    ...
    if (m_activeHdrMode != HdrMode::UNKNOWN)
    {
        ...
        if (m_activeHdrMode != detectedMode)
        {
            ...
            AutoSwapLog(L"CopySample: Pipeline/content mismatch: pipeline=%s content=%s",
                        m_activeHdrMode == HdrMode::HDR ? L"HDR" : L"SDR", detectedHDR ? L"HDR" : L"SDR");
            m_activeHdrMode = HdrMode::UNKNOWN;
            m_pFilter->TriggerMediaTypeChange();
    ...
    // FIXED: Declare hr variable for the rest of the method
    HRESULT hr = S_OK;
    ...
    if (CComQIPtr < IMediaSampleD3D11 > pMSD3D11 = pSample)
    {
        ...
        if (m_D3D11VP.IsReady())
        {
            m_pDeviceContext->CopySubresourceRegion(m_D3D11VP.GetNextInputTexture(m_SampleFormat), 0, 0, 0, 0,
                                                    pD3D11Texture2D, ArraySlice, &srcBox);
        }
        else
        {
            m_pDeviceContext->CopySubresourceRegion(m_TexSrcVideo.pTexture, 0, 0, 0, 0, pD3D11Texture2D, ArraySlice,
                                                    &srcBox);
        }
    }
    else
...
void CDX11VideoProcessor::UpdateTexures()
{
    ...
    if (m_D3D11VP.IsReady())
    {
        if (m_bVPScaling)
        {
            ...
            hr = m_TexConvertOutput.CheckCreate(m_pDevice, m_D3D11OutputFmt, texsize.cx, texsize.cy,
                                                Tex2D_DefaultShaderRTarget);
        }
        else
        {
            hr = m_TexConvertOutput.CheckCreate(m_pDevice, m_D3D11OutputFmt, m_srcRectWidth, m_srcRectHeight,
                                                Tex2D_DefaultShaderRTarget);
        }
    }
    else
    {
        hr = m_TexConvertOutput.CheckCreate(m_pDevice, m_InternalTexFmt, m_srcRectWidth, m_srcRectHeight,
                                            Tex2D_DefaultShaderRTarget);
    }
}
...
void CDX11VideoProcessor::UpdatePostScaleTexures()
{
    ...
    HRESULT hr = m_TexsPostScale.CheckCreate(m_pDevice, m_InternalTexFmt, m_windowRect.Width(), m_windowRect.Height(),
                                             numPostScaleSteps);
    //UpdateStatsPostProc();
}
...
void CDX11VideoProcessor::UpdateUpscalingShaders()
{
    ...
    {
        EXECUTE_ASSERT(
            S_OK == CreatePShaderFromResource(&m_pShaderUpscaleX, s_Upscaling11ResIDs[m_iUpscaling].shaderX));
        ...
        {
            EXECUTE_ASSERT(
                S_OK == CreatePShaderFromResource(&m_pShaderUpscaleY, s_Upscaling11ResIDs[m_iUpscaling].shaderY));
        }
    }
...
void CDX11VideoProcessor::UpdateDownscalingShaders()
{
    ...
    EXECUTE_ASSERT(
        S_OK == CreatePShaderFromResource(&m_pShaderDownscaleX, s_Downscaling11ResIDs[m_iDownscaling].shaderX));
    EXECUTE_ASSERT(
        S_OK == CreatePShaderFromResource(&m_pShaderDownscaleY, s_Downscaling11ResIDs[m_iDownscaling].shaderY));
...
HRESULT CDX11VideoProcessor::UpdateConvertColorShader()
{
    ...
    int convertType = (m_bConvertToSdr && !(m_bHdrSupport && (m_bHdrPassthrough || m_bHdrLocalToneMapping)))
                          ? SHADER_CONVERT_TO_SDR
                          : (m_bHdrSupport && (m_bHdrPassthrough || m_bHdrLocalToneMapping) && m_srcExFmt.
    ...
    if (S_OK == hr)
    {
        hr = m_pDevice->CreatePixelShader(pShaderCode->GetBufferPointer(), pShaderCode->GetBufferSize(), nullptr,
                                          &m_pPSConvertColor);
        pShaderCode->Release();
    }
...
HRESULT CDX11VideoProcessor::D3D11VPPass(ID3D11Texture2D* pRenderTarget, const CRect& srcRect, const CRect& dstRect,
                                         const bool second)
{
...
HRESULT CDX11VideoProcessor::ResizeShaderPass(const Tex2D_t& Tex, ID3D11Texture2D* pRenderTarget, const CRect& srcRect,
                                              const CRect& dstRect, const int rotation)
{
    HRESULT hr = S_OK;
    ...
    if (resizerX && resizerY)
    {
        ...
        if (!m_TexResize.pTexture)
        {
            ...
            hr = m_TexResize.Create(m_pDevice, DXGI_FORMAT_R16G16B16A16_FLOAT, texWidth, texHeight,
                                    Tex2D_DefaultShaderRTarget);
            if (FAILED(hr))
            {
                DLog(L"CDX11VideoProcessor::ResizeShaderPass() : m_TexResize.Create() failed with error {}",
                     HR2Str(hr));
                return hr;
            }
        }
...
HRESULT CDX11VideoProcessor::FinalPass(const Tex2D_t& Tex, ID3D11Texture2D* pRenderTarget, const CRect& srcRect,
                                       const CRect& dstRect)
{
    CComPtr < ID3D11RenderTargetView > pRenderTargetView;
    ...
    TextureBlt11(m_pDeviceContext, pRenderTargetView, VP, m_pVSimpleInputLayout, m_pVS_Simple, m_pPSFinalPass,
                 Tex.pShaderResource, m_pSamplerPoint, m_pFinalPassConstantBuffer, m_pVertexBuffer);
    ...
    if (m_pFilter->m_pSub11CallBack)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = m_pFilter->m_pSub11CallBack->Render11(rtStart, 0, m_rtAvgTimePerFrame, rDstVid, rDstVid, rSrcPri,
                                                       1., m_iStereo3dTransform == 1
                                                               ? m_nStereoSubtitlesOffsetInPixels
                                                               : 0);
                                     ...
                                     const bool second)
{
    ...
    if (m_D3D11VP.IsReady())
    {
        if (!(m_iSwapEffect == SWAPEFFECT_Discard && (m_VendorId == PCIV_AMDATI || m_VendorId == PCIV_INTEL)))
        {
            const bool bNeedShaderTransform =
                (m_TexConvertOutput.desc.Width != dstRect.Width() || m_TexConvertOutput.desc.Height != dstRect.Height()
                    || m_bFlip
                    || dstRect.right > m_windowRect.right || dstRect.bottom > m_windowRect.bottom)
                || (m_bHdrSupport && (m_bHdrPassthrough || m_bHdrLocalToneMapping));
            // At least on Nvidia we can sometimes get the "D3D11: Removing Device" error here when HDR Passthrough.
    ...
    if (numSteps)
    {
        ...
        {
            StepSetting();
            hr = TextureCopyRect(*pInputTexture, pRT, rect, rect, m_pPSHDR10ToneMapping, m_pHDR10ToneMappingConstants,
                                 0, false);
        }
        else
        {
        ...
        if (m_pPostScaleShaders.size())
        {
            ...
            {
                StepSetting();
                hr = TextureCopyRect(*pInputTexture, pRT, rect, rect, m_pPostScaleShaders[idx].shader,
                                     m_pPostScaleConstants, 0, false);
            }
        }
        ...
        if (m_pPSHalfOUtoInterlace)
        {
            ...
            hr = TextureCopyRect(*pInputTexture, pRT, rect, rect, m_pPSHalfOUtoInterlace,
                                 m_pHalfOUtoInterlaceConstantBuffer, 0, false);
        }
...
HRESULT CDX11VideoProcessor::GetCurentImage(long* pDIBImage)
{
    ...
    const bool bisHDROutput = (m_bHdrSupport && (m_bHdrPassthrough || m_bHdrLocalToneMapping) && SourceIsHDR());
    ...
    if (bisHDROutput)
    {
        if (m_D3D11VP.IsReady())
        {
            ...
            auto resId = m_srcExFmt.VideoTransferFunction == MFVideoTransFunc_2084
                             ? IDF_PS_11_CONVERT_PQ_TO_SDR
                             : IDF_PS_11_FIXCONVERT_HLG_TO_SDR;
            EXECUTE_ASSERT(S_OK == CreatePShaderFromResource(&m_pPSCorrection, resId));
            SetShaderLuminanceParams();
...
HRESULT CDX11VideoProcessor::GetDisplayedImage(BYTE** ppDib, unsigned* pSize)
{
    ...
    *pSize = sizeof(BITMAPINFOHEADER) + dib_size;
    ...
    if (SUCCEEDED(hr))
    {
        pConvertToDibFunc(desc.Height, (BYTE*)(pBIH + 1), dib_pitch, (BYTE*)mappedResource.pData,
...
HRESULT CDX11VideoProcessor::GetVPInfo(std::wstring& str)
{
    ...
    if (m_D3D11VP.IsReady())
    {
        ...
        if (rateConvCaps.ProcessorCaps & D3D11_VIDEO_PROCESSOR_PROCESSOR_CAPS_FRAME_RATE_CONVERSION)
            str.append(
                L" Frame Rate Conversion");
        str_trim_end(str, ',');
        str += std::format(L"\nReference Frames: Past {}, Future {}", rateConvCaps.PastFrames,
    ...
    str += std::format(L"\nSource rect    : {},{},{},{} - {}x{}", m_srcRect.left, m_srcRect.top, m_srcRect.right,
                       m_srcRect.bottom, m_srcRect.Width(), m_srcRect.Height());
    str += std::format(L"\nVideo rect     : {},{},{},{} - {}x{}", m_videoRect.left, m_videoRect.top, m_videoRect.right,
                       m_videoRect.bottom, m_videoRect.Width(), m_videoRect.Height());
    str += std::format(L"\nWindow rect    : {},{},{},{} - {}x{}", m_windowRect.left, m_windowRect.top,
                       m_windowRect.right, m_windowRect.bottom, m_windowRect.Width(), m_windowRect.Height());
...
void CDX11VideoProcessor::Configure(const Settings_t& config)
{
    ...
    {
        m_iChromaScaling = config.iChromaScaling;
        changeConvertShader = m_PSConvColorData.bEnable && (m_srcParams.Subsampling == 420 || m_srcParams.Subsampling ==
            422);
    }
    ...
    if (changeSuperRes)
    {
        auto superRes = (m_bVPScaling && (m_srcParams.CDepth == 8 || !m_bACMEnabled))
                            ? m_iVPSuperRes
                            : SUPERRES_Disable;
        m_bVPUseSuperRes = (m_D3D11VP.SetSuperRes(superRes) == S_OK);
    }
...
HRESULT CDX11VideoProcessor::AddPreScaleShader(const std::wstring& name, const std::string& srcCode)
{
    ...
    if (S_OK == hr)
    {
        ...
        hr = m_pDevice->CreatePixelShader(pShaderCode->GetBufferPointer(), pShaderCode->GetBufferSize(), nullptr,
                                          &m_pPreScaleShaders.back().shader);
        if (S_OK == hr)
        {
...
HRESULT CDX11VideoProcessor::AddPostScaleShader(const std::wstring& name, const std::string& srcCode)
{
    ...
    if (S_OK == hr)
    {
        ...
        hr = m_pDevice->CreatePixelShader(pShaderCode->GetBufferPointer(), pShaderCode->GetBufferSize(), nullptr,
                                          &m_pPostScaleShaders.back().shader);
        if (S_OK == hr)
        {
...
void CDX11VideoProcessor::UpdateStatsStatic()
{
    ...
    {
        m_strStatsHeader = std::format(L"MPC VR {}, Direct3D 11, Windows {}", _CRT_WIDE(VERSION_STR),
                                       GetWindowsVersion());
            ...
            if (m_bHdrSupport && (m_bHdrPassthrough || m_bHdrLocalToneMapping))
            {
                ...
                else if (m_bHdrLocalToneMapping)
                {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        m_strStatsHDR.append(L" ACEScg");
                        break;
                    default:
                        m_strStatsHDR.append(L" Unknown");
                        break;
                    }
                ...
                if (m_lastHdr10.bValid)
                {
                    ...
                    m_strStatsHDR += std::format(L"\n Mastering {}/{} nits",
                                                 m_lastHdr10.hdr10.MinMasteringLuminance / 10000,
                    ...
                    m_strStatsHDR += std::format(L"\n RED {} {}", m_lastHdr10.hdr10.RedPrimary[0],
                                                 m_lastHdr10.hdr10.RedPrimary[1]);
                    m_strStatsHDR += std::format(L"\n GREEN {} {}", m_lastHdr10.hdr10.GreenPrimary[0],
                                                 m_lastHdr10.hdr10.GreenPrimary[1]);
                    m_strStatsHDR += std::format(L"\n BLUE {} {}", m_lastHdr10.hdr10.BluePrimary[0],
                                                 m_lastHdr10.hdr10.BluePrimary[1]);
                    m_strStatsHDR += std::format(L"\n White {} {}", m_lastHdr10.hdr10.WhitePoint[0],
                                                 m_lastHdr10.hdr10.WhitePoint[1]);
                }
                if (m_hdr10.bValid)
                {
                    m_strStatsHDR += std::format(L"\n Timestamp {}", m_hdr10.timestamp);
                    m_strStatsHDR += std::format(L"\n Mastering {}/{} nits",
                                                 m_hdr10.hdr10.MinMasteringLuminance / 10000,
                                                 m_hdr10.hdr10.MaxMasteringLuminance);
                    ...
                    m_strStatsHDR += std::format(L"\n RED {} {}", m_hdr10.hdr10.RedPrimary[0],
                                                 m_hdr10.hdr10.RedPrimary[1]);
                    m_strStatsHDR += std::format(L"\n GREEN {} {}", m_hdr10.hdr10.GreenPrimary[0],
                                                 m_hdr10.hdr10.GreenPrimary[1]);
                    m_strStatsHDR += std::format(L"\n BLUE {} {}", m_hdr10.hdr10.BluePrimary[0],
                                                 m_hdr10.hdr10.BluePrimary[1]);
                    m_strStatsHDR += std::format(L"\n White {} {}", m_hdr10.hdr10.WhitePoint[0],
                                                 m_hdr10.hdr10.WhitePoint[1]);
                }
            }
            else if (m_bConvertToSdr)
            {
...
HRESULT CDX11VideoProcessor::DrawStats(ID3D11Texture2D* pRenderTarget)
{
    ...
    if (m_iRotation)
    {
        str += std::format(L"\nScaling       : {}x{} r{}\u00B0> {}x{}", m_srcRectWidth, m_srcRectHeight, m_iRotation,
                           dstW, dstH);
    }
    else
    {
    ...
    str += std::format(L"\nFrames: {:5}, skipped: {}/{}, failed: {}",
                       m_pFilter->m_FrameStats.GetFrames(), m_pFilter->m_DrawStats.m_dropped, m_RenderStats.dropped2,
                       m_RenderStats.failed);
    ...
    if (S_OK == hr)
    {
        ...
        hr = m_Font3D.Draw2DText(pRenderTargetView, rtSize, m_StatsTextPoint.x, m_StatsTextPoint.y, m_dwStatsTextColor,
                                 str.c_str());
        ...
        m_Rect3D.Set({col, m_StatsRect.bottom - 11, col + 5, m_StatsRect.bottom - 1}, rtSize,
...
STDMETHODIMP CDX11VideoProcessor::SetProcAmpValues(DWORD dwFlags, DXVA2_ProcAmpValues* pValues)
{
    ...
    {
        m_DXVA2ProcAmpValues.Brightness.ll = std::clamp(pValues->Brightness.ll, m_DXVA2ProcAmpRanges[0].MinValue.ll,
                                                        m_DXVA2ProcAmpRanges[0].MaxValue.ll);
    }
    if (dwFlags & DXVA2_ProcAmp_Contrast)
    {
        m_DXVA2ProcAmpValues.Contrast.ll = std::clamp(pValues->Contrast.ll, m_DXVA2ProcAmpRanges[1].MinValue.ll,
                                                      m_DXVA2ProcAmpRanges[1].MaxValue.ll);
    }
    if (dwFlags & DXVA2_ProcAmp_Hue)
    {
        m_DXVA2ProcAmpValues.Hue.ll = std::clamp(pValues->Hue.ll, m_DXVA2ProcAmpRanges[2].MinValue.ll,
                                                 m_DXVA2ProcAmpRanges[2].MaxValue.ll);
    }
    if (dwFlags & DXVA2_ProcAmp_Saturation)
    {
        m_DXVA2ProcAmpValues.Saturation.ll = std::clamp(pValues->Saturation.ll, m_DXVA2ProcAmpRanges[3].MinValue.ll,
                                                        m_DXVA2ProcAmpRanges[3].MaxValue.ll);
    }
...
STDMETHODIMP CDX11VideoProcessor::SetAlphaBitmap(const MFVideoAlphaBitmap* pBmpParms)
{
    ...
    if (pBmpParms->GetBitmapFromDC && pBmpParms->bitmap.hdc)
    {
        ...
        hr = m_TexAlphaBitmap.CheckCreate(m_pDevice, DXGI_FORMAT_B8G8R8A8_UNORM, bm.bmWidth, bm.bmHeight,
                                          Tex2D_DefaultShader);
...
STDMETHODIMP CDX11VideoProcessor::UpdateAlphaBitmapParameters(const MFVideoAlphaBitmapParams* pBmpParms)
{
    ...
    if (m_bAlphaBitmapEnable)
    {
        ...
        {
            HRESULT hr = CreateVertexBuffer(m_pDevice, &m_pAlphaBitmapVertex, m_TexAlphaBitmap.desc.Width,
                                            m_TexAlphaBitmap.desc.Height, m_AlphaBitmapRectSrc, 0, false);
            if (FAILED(hr))
            {
                m_bAlphaBitmapEnable = false;
                return hr;
