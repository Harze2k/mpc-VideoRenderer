...
CMpcVideoRenderer::CMpcVideoRenderer(LPUNKNOWN pUnk, HRESULT *phr)
    : CBaseVideoRenderer2(__uuidof(this), L"MPC Video Renderer", pUnk, phr), m_Sets(),m_filterState(State_Stopped), m_bFlushing(false), m_bValidBuffer(false), m_hWnd(nullptr), m_hWndWindow(nullptr), m_hWndParent(nullptr), m_hWndDrain(nullptr), m_hWndParentMain(nullptr), m_hMon(nullptr), m_bPrimaryDisplay(false), m_Stepping(0), m_rtStartTime(0), m_pSubCallBack(nullptr), m_pSub11CallBack(nullptr), m_bForceRedrawing(true), m_bEnableFullscreenControl(false), m_bDisplayModeChanging(false), m_bSetNewMediaTypeToInputPin(false), m_bIsFullscreen(false), m_bIsD3DFullscreen(false)
{
    DLog(L"CMpcVideoRenderer::CMpcVideoRenderer()");
    auto nPrevInstance = g_nInstance++;
    if (nPrevInstance > 0)
    {
        *phr = E_ABORT;
    ...
    if (ERROR_SUCCESS == key.Open(HKEY_CURRENT_USER, OPT_REGKEY_VIDEORENDERER, KEY_READ))
    {
        ...
        {
            m_Sets.iHdrLocalToneMappingType = discard<int>(dw, 1, 1, 5); // Updated max to 5 for ACEScg
        }
        float value = 0;
...
HRESULT CMpcVideoRenderer::SetMediaType(const CMediaType *pmt)
{
    ...
    if (!inputPin->FrameInVideoMem())
    {
        ...
        if (mtNew != mt)
        {
            ...
            {
                DLog(
                    L"CMpcVideoRenderer::SetMediaType() : upstream filter accepted new media type. QueryAccept return S_OK");
    ...
    {
        if (aspectNew != aspect || framesizeNew != framesize || aspectNew != m_videoAspectRatio || framesizeNew != m_videoSize)
        {
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!pThis)
    {
        if ((uMsg != WM_NCCREATE) || (nullptr == (pThis = (CMpcVideoRenderer *)((LPCREATESTRUCTW)lParam)->lpCreateParams)))
        {
...
STDMETHODIMP_(void)
CMpcVideoRenderer
    ::GetSettings(Settings_t &setings)
{
    setings = m_Sets;
}
STDMETHODIMP_(void)
CMpcVideoRenderer
    ::SetSettings(const Settings_t &setings)
{
    CAutoLock cRendererLock(&m_RendererLock);
    m_Sets = setings;
...
STDMETHODIMP CMpcVideoRenderer::Flt_GetInt64(LPCSTR field, __int64 *value)
{
    ...
    {
        *value = ((uint64_t)VER_MAJOR << 48) | ((uint64_t)VER_MINOR << 32) | ((uint64_t)VER_BUILD << 16) | ((uint64_t)REV_NUM);
        return S_OK;
    }
    return E_INVALIDARG;
}
...
STDMETHODIMP CMpcVideoRenderer::DeliverFrame(REFERENCE_TIME start, REFERENCE_TIME stop, LPVOID context,
