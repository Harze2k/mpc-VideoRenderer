...
void CVRMainPPage::SetControls()
{
    ...
    else if (m_SetsPP.bHdrPassthrough)
    {
        ComboBox_SelectByItemData(m_hWnd, IDC_COMBO9, 0); // "Passthrough"
    }
    ...
    {
        // Auto-detect based on HDR settings
        if (!m_SetsPP.bHdrPassthrough && !m_SetsPP.bHdrLocalToneMapping)
        {
            m_SetsPP.fHdrDisplayMaxNits = 400.0f; // SDR content
        }
        else
        {
            m_SetsPP.fHdrDisplayMaxNits = 1000.0f; // HDR content
        }
    }
    wchar_t buffer[32] = {};
    ...
    if (uMsg == WM_COMMAND)
    {
        ...
        if (action == BN_CLICKED)
        {
            ...
            if (nID == IDC_CHECK5)
            {
                ...
                SetDirty();
                GetDlgItem(IDC_STATIC7).EnableWindow(
                    m_SetsPP.bVPScaling && m_SetsPP.bUseD3D11 && IsWindows10OrGreater());
                GetDlgItem(IDC_COMBO8).
                    EnableWindow(m_SetsPP.bVPScaling && m_SetsPP.bUseD3D11 && IsWindows10OrGreater());
                return (LRESULT)1;
            }
            if (nID == IDC_CHECK6)
            {
