        ...
        if (IsWindows8OrGreater())
        {
            ...
            if (hShcore)
            {
                ...
                *tpGetDpiForMonitor
                ...
                (HMONITOR
                hmonitor, MONITOR_DPI_TYPE
                dpiType, UINT * dpiX, UINT * dpiY
                )
                ;
                static tpGetDpiForMonitor pGetDpiForMonitor = (tpGetDpiForMonitor)GetProcAddress(
                    hShcore, "GetDpiForMonitor");
                if (pGetDpiForMonitor)
                {
                    UINT dpix, dpiy;
                    if (S_OK == pGetDpiForMonitor(MonitorFromWindow(m_hWnd, MONITOR_DEFAULTTONEAREST),
...
bool CVideoProcessor::CheckDoviMetadata(const MediaSideDataDOVIMetadata* pDOVIMetadata, const uint8_t maxReshapeMethon)
{
    ...
    for (const auto& curve : pDOVIMetadata->Mapping.curves)
    {
        ...
        {
            if (curve.mapping_idc[i] > maxReshapeMethon)
            {
                // 0 polynomial, 1 mmr
                return false;
            }
...
STDMETHODIMP CVideoProcessor::GetProcAmpRange(DWORD dwProperty, DXVA2_ValueRange* pPropRange)
{
    ...
    switch (dwProperty)
    {
    case DXVA2_ProcAmp_Brightness: *pPropRange = m_DXVA2ProcAmpRanges[0];
        break;
    case DXVA2_ProcAmp_Contrast: *pPropRange = m_DXVA2ProcAmpRanges[1];
        break;
    case DXVA2_ProcAmp_Hue: *pPropRange = m_DXVA2ProcAmpRanges[2];
        break;
    case DXVA2_ProcAmp_Saturation: *pPropRange = m_DXVA2ProcAmpRanges[3];
        break;
    default:
        return E_INVALIDARG;
    }
